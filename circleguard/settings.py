...
later_replay_link = (
    '<a href="https://osu.ppy.sh/scores/osu/{r.later_replay.replay_id}">'
    "{r.later_replay.username} +{later_replay_mods_short_name}</a>"
)
earlier_replay_link = (
    '<a href="https://osu.ppy.sh/scores/osu/{r.earlier_replay.replay_id}">'
    "{r.earlier_replay.username} +{earlier_replay_mods_short_name}</a>"
)
sim_map_link = (
    '<a href="https://osu.ppy.sh/b/{r.replay1.map_id}">{r.replay1.map_id}</a>'
)
...
message_correction_found = (
    f"[{{ts:%X}}] {replay_link} on map {map_link}. Snaps:\n{{snaps}}"
)
message_timewarp_found = (
    f"[{{ts:%X}}] {{frametime:.1f}} avg cv frametime. {replay_link} on map {map_link}"
)
...
DEFAULTS = {
    "Messages": {
        ...
        "message_timewarp_found_display": message_timewarp_found
        + " (not below threshold)",
    ...
    "Templates": {
        "template_steal": (
            "[osu!std] {r.later_replay.username} | Replay Stealing"
            "\n\n"
        ...
        "template_relax": (
            "[osu!std] {r.replay.username} | Relax"
            "\n\n"
        ...
        "template_correction": (
            "[osu!std] {r.replay.username} | Aim Correction"
            "\n\n"
            "Profile: https://osu.ppy.sh/users/{r.replay.user_id}"
            "\n\n"
            "replay: https://osu.ppy.sh/scores/osu/{r.replay.replay_id}"
        ...
        "template_timewarp": (
            "[osu!std] {r.replay.username} | Timewarp"
            "\n\n"
            "Profile: https://osu.ppy.sh/users/{r.replay.user_id}"
            "\n\n"
            "replay: https://osu.ppy.sh/scores/osu/{r.replay.replay_id}"
    ...
    "Loadables": {"default_span_map": "1-50", "default_span_user": ""},
    "Thresholds": {
        "steal_max_sim": 15,
        "steal_max_sim_display": 16,
        "relax_max_ur": 50,
    ...
    "Tutorial": {"tutorial_drag_loadables_seen": False},
    "Misc": {
        "frametime_graph_display": "cv",
        "frametime_graph_display_options": {"ucv": "ucv", "cv": "cv"},
    ...
    "Experimental": {"rainbow_accent": False},
    "Core": {
        "ran": False,
...
FORCE_UPDATE = {
    ...
    "2.1.1": ["speed_options"],
    "2.4.0": ["message_relax_found", "message_relax_found_display", "template_relax"],
    "2.5.0": ["log_level"],
    "2.6.0": [
    ...
    "2.7.0": ["message_starting_investigation_analysis"],
    "2.8.0": [
    ...
    "2.11.0": ["message_starting_analysis", "template_steal"],
    "2.11.1": ["message_correction_snaps"],
    "2.12.0": ["message_loading_replays"],
    "2.12.2": [
        "template_steal",
        "template_relax",
        "template_correction",
...
class LinkableSetting:
    ...
    def on_setting_changed_from_gui(self, setting, value):
        ...
        if setting not in self.settings:
            raise ValueError(
                f"expected setting to be one of the subscribed "
...
def overwrite_config():
    ...
    for setting in keys:
        ...
        if (
            config[section] == {}
            and section in COMMENTS
            and "section" in COMMENTS[section]
        ):
        ...
        if section in COMMENTS and setting in COMMENTS[section]:
            ...
            config[section][
                comment
            ] = None  # setting a configparser key to None writes it as is, without a trailing = for the val
